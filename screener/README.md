...
実データ検証をManus等の自律エージェントやVPSで走らせる場合の段階別指示書を
[`docs/manus_prompt.md`](docs/manus_prompt.md) に用意した。疎通確認 → ユニバース構築
→ 本番 → 悲観シナリオの4段階に分け、各段階で停止・報告させる構成になっている
（8時間走らせた後で株価取得が失敗していたと分かるのを避けるため）。
