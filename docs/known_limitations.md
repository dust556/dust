...
| L-06 | The EA itself cannot inject slippage into the MT5 tester. | **Resolved by v0.4.1a Addendum D and implemented in Patch-2**: stress is evaluated offline by `tools/g3research/stress_replay.py`, gated by the Base parity check. The EA's own `InpStressExtraSpreadPoints` remains a coarse in-terminal aid only. |
...
| L-08 | `fakeout_3` / `fakeout_6` are a tri-state (TRUE / FALSE / NA) per v0.4.1a Addendum E. NA appears whenever the six bar window could not be fully observed, so the diagnostic is honest about restarts and missing ticks rather than defaulting to FALSE. | Implemented in Patch-2. |
...
| L-10 | Leaving `STATE_UNCERTAIN` requires a named operator, and a new state epoch additionally requires a flat book and a G1 review record. This is deliberate friction, not an oversight: an unattended EA in STATE_UNCERTAIN will keep managing its open positions and will never resume trading by itself. | Implemented in Patch-2 per Addendum B. |
...
| L-12 | The offline research instruments in `tools/g3research/` are engines, not results. They have been exercised on synthetic fixtures only, because no feed, symbol specification, initial equity or account currency has been supplied. Their numbers become meaningful only once CR-009 is answered. | Addendum D states plainly that the replay engine is a research instrument and not part of the live EA. |
| L-13 | The offline replay re-implements the shared state machine in Python. `tools/g3research/spec_constants.py` checks every constant back against the MQL5 `#define`s and raises on drift, and the Addendum D / F parity gates exist precisely to catch a behavioural divergence that constants alone cannot. | Regression P-000. |
