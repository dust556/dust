...
| I-030 | test_state_uncertain_keeps_protective_management | Corrupt both state stores with an open managed position, then restart. | Open positions keep being managed (trail / partial / timeout still act on the reconstructible record), NO new order is sent, no EVAL row is produced for new bars, and the EA does not flatten the book. | NOT_RUN |
| I-031 | test_state_uncertain_writes_no_state | Same conditions, then inspect the state file and global variables. | Neither store is rewritten while STATE_UNCERTAIN holds, so an untrusted peak is never cemented. | NOT_RUN |
| I-032 | test_manual_recovery_is_never_automatic | Leave the EA in STATE_UNCERTAIN with the default inputs, then rerun with each recovery mode. | With `InpManualRecovery=NONE` nothing changes. RECONCILE writes evidence only. NEW_EPOCH is refused without an operator id, without a G1 review record, or with an open position; when admitted it rebases the peak, keeps an unknown hard stop latched, audits everything and requires a restart before trading. | NOT_RUN |
| I-033 | test_shadow_record_precedes_portfolio_guard | Force a portfolio rejection (three positions already open). | A `SHADOW` row exists for the rejected candidate with the full strategy context, followed by an `EVAL` row carrying the portfolio reason. | NOT_RUN |
| I-034 | test_fakeout_window_survives_an_early_exit | Exit mode A, a trade that is closed by TP inside two bars. | The `FAKEOUT` row still arrives after the sixth bar, with TRUE/FALSE decided from the price path rather than from the exit. | NOT_RUN |
| I-035 | test_fakeout_restart_yields_na | Restart the terminal inside a fakeout window. | The restored window reports NA, never FALSE. | NOT_RUN |
