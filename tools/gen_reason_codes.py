#!/usr/bin/env python3
"""Generate schema/reason_codes.md from ENUM_G3_REASON in src/G3Types.mqh."""
import os
import re
import sys

ROOT = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
SRC = os.path.join(ROOT, "src", "G3Types.mqh")
OUT = os.path.join(ROOT, "schema", "reason_codes.md")
...
def main():
    ...
    names = re.findall(r'case G3_R_[A-Z0-9_]+:\s*return\("([A-Z0-9_]+)"\);', text)
    names.append("UNMAPPED")
    missing = [n for n in names if n not in DESC]
    if missing:
        print("reason codes without a description: %s" % missing)
        return 1
    lines = ["# G3 Research Log Reason Codes", "",
    ...
    for n in names:
        sec, desc = DESC[n]
        lines.append("| `%s` | %s | %s |" % (n, sec, desc))
    lines.append("")
    open(OUT, "w", encoding="utf-8").write("\n".join(lines))
    print("wrote %s (%d codes)" % (OUT, len(names)))
    return 0
